...
namespace Hungarian {
    ...
    std::vector<size_t> Solve(const Matrix &input, MODE mode) {
        Result result;
        result.success = true;
        ...
        /*if (result.success) {
            std::cout << "Successfully found a solution." << std::endl;
        } else {
        ...
        for (size_t i = 0; i < result.assignment.size(); ++i) {
            for (size_t j = 0; j < result.assignment[i].size(); ++j) {
                if (result.assignment[i][j] == 1) {
                    tuples.push_back(j);
                }
            }
        }
